...
impl BootInfoFrameAllocator {
    ...
    ///
    /// This function is unsafe because the caller must huarantee that the passed
    /// memory_map is valid. The main requirement is that all frames that are
    ...
    // return some type which implements the Iterator trait with item type PhysFrame
...
unsafe impl FrameAllocator<Size4KiB> for EmptyFrameAllocator {
    // unsafe because the implementer must guarantee that the allocator yields only unused frames
    fn allocate_frame(&mut self) -> Option<PhysFrame> {
        None
    }
}
